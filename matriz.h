...
public:
	...
	static matrix GenScaleInverse(vector<float> s_axis, int size)
	{
		vector<vector<float>> m_scale(size, vector<float>(size, 0));

		for (int i = 0; i < size; ++i)
		{
			if (i < s_axis.size())
			{
				m_scale[i][i] = 1.0 / s_axis[i];
			}
			else
			{
				m_scale[i][i] = 1;
	...
	static matrix GenTranslationInverse(vector<float> t_axis, int size)
	{
		vector<vector<float>> m_translation(size, vector<float>(size, 0));
		...
		for (int i = 0; i < size; ++i)
		{
			...
			if (i < t_axis.size())
			{
				m_translation[i][size - 1] = -t_axis[i];
			}
		}

		matrix TranslationInverse(m_translation);
	...
	static matrix GenRotateInverseX(double angle, int size)
	{
		double radian_angle = -angle * PI / 180.0;  // Cambio de signo para la rotación inversa

		double _cos = cos(radian_angle);
		double _sin = sin(radian_angle);

		matrix RotateInverseX = GenIdentity(size);

		RotateInverseX.m[1][1] = _cos;
		RotateInverseX.m[1][2] = _sin;
		RotateInverseX.m[2][1] = -_sin;
		RotateInverseX.m[2][2] = _cos;

		return RotateInverseX;
	}
	static matrix GenRotateInverseY(double angle, int size)
	{
		double radian_angle = -angle * PI / 180.0;  // Cambio de signo para la rotación inversa
		...
		matrix RotateInverseY = GenIdentity(size);

		RotateInverseY.m[0][0] = _cos;
		RotateInverseY.m[0][2] = -_sin;
		RotateInverseY.m[2][0] = _sin;
		RotateInverseY.m[2][2] = _cos;

		return RotateInverseY;
	}
	static matrix GenRotateInverseZ(double angle, int size)
	{
		double radian_angle = -angle * PI / 180.0;  // Cambio de signo para la rotación inversa
		...
		matrix RotateInverseZ = GenIdentity(size);

		RotateInverseZ.m[0][0] = _cos;
		RotateInverseZ.m[0][1] = _sin;
		RotateInverseZ.m[1][0] = -_sin;
		RotateInverseZ.m[1][1] = _cos;

		return RotateInverseZ;
	}
